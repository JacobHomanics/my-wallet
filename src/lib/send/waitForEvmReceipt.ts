 ...
 * Needed between back-to-back EVM sends on the same network so the next tx
 * does not reuse the prior nonce ("replacement transaction underpriced").
 */
export async function waitForEvmReceipt(
  ...
  while (Date.now() < deadline) {
    ...
    if (receipt != null && typeof receipt === 'object') {
      ...
      // status 0x0 = reverted; still frees the nonce for the next send.
      if (status === '0x0') {
        throw new Error(
